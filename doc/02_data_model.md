...
| 顧客担当者 | `contact` | `/crm/customer/{cid}/contact/{ctid}` | 顧客側の担当者（顧客企業の人物） |
| 担当営業 | `member` | `/crm/customer/{cid}/member/{uid}` | 自社の担当営業（複数人対応） |
...
```
/crm/customer/{cid}                         ← 顧客
/crm/customer/{cid}/contact/{ctid}          ← 顧客担当者（顧客企業の人物）
/crm/customer/{cid}/activity/{aid}          ← 対応履歴（deal_uri で商談に紐付け可）
/crm/customer/{cid}/member/{uid}            ← 担当営業（自社ユーザー、複数人対応）
/crm/member/{uid}/{cid}                     ← 担当営業のエイリアス（横断検索用）
...
customer (1) ──< member   (N)    ← 顧客エントリの alternate リンク（エイリアス）で管理
...
| `assigned_uid` | string | | 255 | 担当営業の vte.cx UID（**未使用**。担当営業は `/crm/customer/{cid}/member/{uid}` エントリ + エイリアスで管理） |
...
| `next_action_date` | date | | — | 次アクション実施予定日 |
...
| フィールド | 型 | 必須 | 最大 | 説明 |
|---|---|:---:|---:|---|
| `uid` | string | ✓ | 255 | 担当営業の vte.cx UID |
| `is_deleted` | boolean | | — | 論理削除フラグ |

> 実態は**顧客エントリの `link` 配列に `rel="alternate"` を追加する**ことで管理する。  
> エイリアス `/crm/member/{uid}/{cid}` を顧客エントリに付与することで、`getFeed('/crm/member/{uid}')` による横断検索が可能になる。
